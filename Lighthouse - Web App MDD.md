...
2. **Player Setup** — Choose the number of players (1–6) and optionally name each player. **2–6** players share the device (pass and play); **1** player is a solo game against the computer opponent "Skipper". "Start Game" begins with Player 1's turn.
...
- Number stepper for player count (1–6) and an optional name field per player. At 1 player the screen says it's a solo game against the bot "Skipper" (marked with a robot icon); the single name field is the human's. "Start Game" button.
...
- [ ] Player setup accepts 1–6 players with optional names (1 = solo against the bot).
- [ ] Solo mode: choosing 1 player in setup pits the human against the bot, which takes its own turns (rolling and banking) with the buttons locked, and follows all the same rules including elimination and the final round.
